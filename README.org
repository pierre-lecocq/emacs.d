...
It works for internal packages and third-party packages installed thanks to =package.el= (through ELPA/MELPA/GNU...)
...
|-------------------------+---------------------+-------------+--------------------------------------------------------------------------------------------------------------------------------|
| =pl/mkpath=               | =&rest args=          | No          | Create a file or a directory. It take named arguments: :name (string), :base (string), :directory (boolean), :create (boolean) |
| =pl/set-locale=           | =locale=              | No          | Set a locale (i.e: 'utf-8 or 'latin-1)                                                                                         |
...
| =pl/transparency=         | =value=               | Yes         | Set transparency to the frame. 0 is invisible and 100 is opaque                                                                |
...
| =pl/kill-buffers-by-mode= | =&optional mode-name= | Yes         | Kill all buffers of a specific major-mode (i.e: dired-mode or ruby-mode)                                                       |
